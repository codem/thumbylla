...
Images generated by Thumbor bypass the standard GD backend provided by Silverstripe and are instead generated and served from a Thumbor server. Original images are uploaded and stored in the ```ASSETS_DIR``` (default: ./assets) in your website.

Silverstripe 3.6+ is supported currently, 4.x is in the works (PR's welcome).
